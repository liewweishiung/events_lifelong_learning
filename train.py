...
def train_cl(model, train_datasets, replay_mode="none", rnt=None, classes_per_task=None,
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        ...
        if isinstance(model, ContinualLearner) and args.habituation:
            habs_counter = model.habituation_container.habituation_counter[
                model.habituation_container.habituation_counter > 0.01].shape[0]
            habs.append(habs_counter)

    # Save habituation counters:
    np.save("./store/results/habs_over_episodes_tasks{}_{}".format(task_copy, args.habituation_decay_rate),
            np.array([i / model.fcE.fcLayer2.linear.out_features for i in habs]))
